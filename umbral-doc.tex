...
\begin{itemize}
	...
	Finally, it outputs the ciphertext $C = (encKey, encData)$.
		...
		\item $\mathsf{DecFrags}(sk_A, \{F_i\}_{i=1}^t, AE)$: On input the secret key $sk_A$ and a set of $t$ fragments of an encapsulated key, each of them labeled as $F_i$, and the encrypted data $encData$, the fragments decryption algorithm $\mathsf{DecFrags}$ first computes $encKey' = \mathsf{Combine}(\{F_i\}_{i=1}^t)$. With this result, it returns the result of $\mathsf{Dec}(sk_A, C')$, where $C' = (encKey', encData)$. % is the byte representation of $epk'$.
